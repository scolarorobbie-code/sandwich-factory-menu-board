...
export default function DealsScreen() {
  ...
  function applyDeal(dealId: string) {
    if (cart.count === 0) {
      Alert.alert("Add items first", "Add something from the menu, then come back to apply this deal at checkout.");
      return;
    }
    nav.navigate("Checkout", { dealId });
  }
        ...
        renderItem={({ item }) => (
          ...
          >
            ...
            <Pressable
              ...
              onPress={() => applyDeal(item.id)}
              accessibilityRole="button"
              accessibilityLabel={`Apply deal: ${item.title}`}
              accessibilityHint="Goes to checkout with this deal applied"
            >
              <Text style={styles.applyBtnText}>Apply at checkout →</Text>
            </Pressable>
          </View>
        )}
      />
...
const styles = StyleSheet.create({
  ...
  applyBtn: { marginTop: 14, backgroundColor: colors.accent, borderRadius: 999, paddingVertical: 10, paddingHorizontal: 20, alignSelf: "flex-start" },
